...
reg [17:0] fractional;
reg old_wsout;
// The Jaguar core runs this block from xvclk, which is derived from the
// 106.363624 MHz system clock as a 1-in-4 enable pulse:
//   xvclk ~= 26.590906 MHz
//
// One serialized I2S bit is emitted per full SCK cycle, not per SCK toggle.
// Because ben only asserts on one SCK polarity, the effective bit period is:
//   bit_cycles = 2 * (p__ + 1)
//
// The fractional accumulator nudges p__ between p and p+1 often enough to
// hit the intended long-term average divider. With the current constants:
//   1x: p = 8, overflow rate = 0x0D7CF / 2^17 = 0.421501...
//       avg p__       = 8.421501...
//       avg bit_cycles= 18.843002...
//       bit clock     = 1.411182 MHz
//       ws toggle     = 88.198876 kHz  (16 bits/channel word)
//       stereo rate   = 44.099438 kHz
//
//   2x: p = 3, overflow rate = 0x16BC7 / 2^17 = 0.710503...
//       avg p__       = 3.710503...
//       avg bit_cycles= 9.421005...
//       bit clock     = 2.822513 MHz
//       ws toggle     = 176.407037 kHz
//       stereo rate   = 88.203519 kHz
//
// That matches the rest of Butch's CD audio timing model:
//   588 stereo samples/frame * 75 frames/second = 44,100 samples/second.
// A 17.64/35.28 kHz interpretation would not agree with the 588-sample
// frame accounting used by the transport logic.
reg old_tld = 0;
always @(posedge sys_clk)
begin
	old_tld <= tld;
...
// Drive hard zero when I2S output is disabled so downstream logic
// cannot observe stale serialized sample bits after STOP.
assign i2txd = i2sen ? qs[15] : 1'b0;
...
assign snd_l[15:0] = i2sen ? dpl[15:0] : 16'h0;

// I2S.NET (241) - snd_r : join
assign snd_r[15:0] = i2sen ? dpr[15:0] : 16'h0;
